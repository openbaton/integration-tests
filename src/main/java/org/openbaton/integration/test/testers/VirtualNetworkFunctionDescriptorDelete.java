 ...
 *
 * This Tester expects a NSD object from it's preceding task and tries to delete the VNFDs of this NSD, filtered by vnfd name and type.
 */
public class VirtualNetworkFunctionDescriptorDelete extends Tester<VirtualNetworkFunctionDescriptor> {
    ...
    protected Object doWork() throws Exception {
        ...
        for (VirtualNetworkFunctionDescriptor vnfd : vnfds) {
            if (!"".equals(vnfdType)) {
                if (!vnfd.getType().equals(vnfdType))
                    continue;
            }

            if (!"".equals(vnfdName)) {
                if (!vnfd.getName().equals(vnfdName))
                    continue;
            }

            log.info("Delete VNFD of type " + vnfdType + " with name " + vnfd.getName() + " and id " + vnfd.getId());
    ...
    private void deleteVnfd(String id) throws Exception {
        try {
            ...
            log.error("Exception during deletion of VirtualNetworkFunctionDescriptor with id: " + id, sdkEx);
            throw sdkEx;
        }
